		...
		{
			{
				...
				// XXX MPOOL
				if err := vector.Append(vec, vs, vec.Length() == 0, testEngineMp); err != nil {
					log.Fatal(err)
				}
				bat.Vecs[0] = vec
			}
...
func CreateT1(db engine.Database) {
	...
	}
	{
		...
		{
			vec := vector.New(0, types.Type{Oid: types.T(types.T_int32), Size: 4, Width: 4, Precision: 0})
			vs := make([]int32, 5)
			vs[0] = 1
			vs[1] = 2
			vs[2] = 2
		...
		{
			vec := vector.New(0, types.Type{Oid: types.T(types.T_int8), Size: 1, Width: 1, Precision: 0})
			vs := make([]int8, 5)
			vs[0] = 1
			vs[1] = 2
			vs[2] = 4
