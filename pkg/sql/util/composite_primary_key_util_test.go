...
func MakeBatch(columnSi int, rowCount int, mp *mpool.MPool) (*batch.Batch, *plan.ColDef, map[int]interface{}) {
	...
	for i := 0; i < idx; i++ {
		bat.Vecs[i] = vector.New(0, types.Type{Oid: randType()})
		randInsertValues(bat.Vecs[i], bat.Vecs[i].GetType().Oid, rowCount, valueCount, i*rowCount, mp)
	}

	bat.Vecs[idx] = vector.New(0, types.Type{Oid: types.T_varchar, Width: types.MaxVarcharLen})
...
func randInsertValues(v *vector.Vector, t types.T, rowCount int, valueCount map[int]interface{}, valueBegin int, mp *mpool.MPool) {
	...
	case types.T_bool:
		...
		vector.Append(v, vs, v.Length() == 0, mp)
	case types.T_int8:
