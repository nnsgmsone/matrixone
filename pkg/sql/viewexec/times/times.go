...
func (ctr *Container) probeH8(is []int, arg *Argument, bat *batch.Batch, proc *process.Process) error {
	...
	for i := int64(0); i < count; i += UnitLimit {
		...
		ctr.pctr.h8.ht.InsertBatch(int(n), ctr.hashs, unsafe.Pointer(&ctr.h8.keys[0]), ctr.inserts, ctr.values)
		for k, ok := range ctr.inserts[:n] {
			...
			for j := range bat.Rs {
				ctr.bat.Rs[j].Fill(ai, o, z/bat.Zs[o], bat.Vecs[is[j]])
			}
			for vi, vps := range values {
				sel := *vps[k]
				v := ctr.views[vi]
				{ // ring fill
					for j, r := range v.bat.Rs {
						ctr.bat.Rs[v.ris[j]].Mul(r, ai, int64(sel), z/v.bat.Zs[sel])
					}
				}
			}
			ctr.bat.Zs[ai] += z
...
func (ctr *Container) probeH24(is []int, arg *Argument, bat *batch.Batch, proc *process.Process) error {
	...
	for i := int64(0); i < count; i += UnitLimit {
			...
			for j, vec := range gvecs {
				if ctr.pctr.freeIndexs[j][0] >= 0 {
					...
					case types.T_char, types.T_varchar:
						...
						for k := int64(0); k < n; k++ {
							if vp := vps[k]; vp == nil {
								copy(unsafe.Slice((*byte)(unsafe.Pointer(&ctr.h24.keys[k])), 8)[ctr.keyOffs[k]:], vData[vOff[0]:vOff[0]+vLen[0]])
								ctr.keyOffs[k] += vLen[0]
							} else {
								copy(unsafe.Slice((*byte)(unsafe.Pointer(&ctr.h24.keys[k])), 8)[ctr.keyOffs[k]:], vData[vOff[*vp]:vOff[*vp]+vLen[*vp]])
								ctr.keyOffs[k] += vLen[*vp]
							}
						}
					}
					...
					case types.T_char, types.T_varchar:
						...
						vLen := vs.Lengths
						for k := int64(0); k < n; k++ {
							copy(unsafe.Slice((*byte)(unsafe.Pointer(&ctr.h24.keys[k])), 8)[ctr.keyOffs[k]:], vData[vOff[i+k]:vOff[i+k]+vLen[i+k]])
							ctr.keyOffs[k] += vLen[i+k]
						}
					}
				}
		...
		ctr.hashs[0] = 0
		copy(ctr.inserts[:n], ctr.zinserts[:n])
		ctr.pctr.h24.ht.InsertBatch(ctr.hashs, ctr.h24.keys[:n], ctr.inserts, ctr.values)
		for k, ok := range ctr.inserts[:n] {
			o := i + int64(k)
			z := bat.Zs[o]
			for vi, vps := range values {
...
func (ctr *Container) probeH32(is []int, arg *Argument, bat *batch.Batch, proc *process.Process) error {
	...
	for i := int64(0); i < count; i += UnitLimit {
			...
			for j, vec := range gvecs {
				if ctr.pctr.freeIndexs[j][0] >= 0 {
					...
					case types.T_char, types.T_varchar:
						...
						for k := int64(0); k < n; k++ {
							if vp := vps[k]; vp == nil {
								copy(unsafe.Slice((*byte)(unsafe.Pointer(&ctr.h32.keys[k])), 8)[ctr.keyOffs[k]:], vData[vOff[0]:vOff[0]+vLen[0]])
								ctr.keyOffs[k] += vLen[0]
							} else {
								copy(unsafe.Slice((*byte)(unsafe.Pointer(&ctr.h32.keys[k])), 8)[ctr.keyOffs[k]:], vData[vOff[*vp]:vOff[*vp]+vLen[*vp]])
								ctr.keyOffs[k] += vLen[*vp]
							}
						}
					}
					...
					case types.T_char, types.T_varchar:
						...
						vLen := vs.Lengths
						for k := int64(0); k < n; k++ {
							copy(unsafe.Slice((*byte)(unsafe.Pointer(&ctr.h32.keys[k])), 8)[ctr.keyOffs[k]:], vData[vOff[i+k]:vOff[i+k]+vLen[i+k]])
							ctr.keyOffs[k] += vLen[i+k]
						}
					}
				}
		...
		ctr.hashs[0] = 0
		copy(ctr.inserts[:n], ctr.zinserts[:n])
		ctr.pctr.h32.ht.InsertBatch(ctr.hashs, ctr.h32.keys[:n], ctr.inserts, ctr.values)
		for k, ok := range ctr.inserts[:n] {
			o := i + int64(k)
			z := bat.Zs[o]
			for vi, vps := range values {
...
func (ctr *Container) probeH40(is []int, arg *Argument, bat *batch.Batch, proc *process.Process) error {
	...
	for i := int64(0); i < count; i += UnitLimit {
			...
			for j, vec := range gvecs {
				if ctr.pctr.freeIndexs[j][0] >= 0 {
					...
					case types.T_char, types.T_varchar:
						...
						for k := int64(0); k < n; k++ {
							if vp := vps[k]; vp == nil {
								copy(unsafe.Slice((*byte)(unsafe.Pointer(&ctr.h40.keys[k])), 8)[ctr.keyOffs[k]:], vData[vOff[0]:vOff[0]+vLen[0]])
								ctr.keyOffs[k] += vLen[0]
							} else {
								copy(unsafe.Slice((*byte)(unsafe.Pointer(&ctr.h40.keys[k])), 8)[ctr.keyOffs[k]:], vData[vOff[*vp]:vOff[*vp]+vLen[*vp]])
								ctr.keyOffs[k] += vLen[*vp]
							}
						}
					}
					...
					case types.T_char, types.T_varchar:
						...
						vLen := vs.Lengths
						for k := int64(0); k < n; k++ {
							copy(unsafe.Slice((*byte)(unsafe.Pointer(&ctr.h40.keys[k])), 8)[ctr.keyOffs[k]:], vData[vOff[i+k]:vOff[i+k]+vLen[i+k]])
							ctr.keyOffs[k] += vLen[i+k]
						}
					}
				}
		...
		ctr.hashs[0] = 0
		copy(ctr.inserts[:n], ctr.zinserts[:n])
		ctr.pctr.h40.ht.InsertBatch(ctr.hashs, ctr.h40.keys[:n], ctr.inserts, ctr.values)
		for k, ok := range ctr.inserts[:n] {
			o := i + int64(k)
			z := bat.Zs[o]
			for vi, vps := range values {
