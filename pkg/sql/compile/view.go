...
func constructViews(bats []*batch.Batch, fvars []string, isBares []bool) {
	for i, isB := range isBares {
		if isB {
			continue
		}
		isBares[i] = constructView(bats[i], fvars[i])
	}
}

func constructView(bat *batch.Batch, fvar string) bool {
	var rows uint64
