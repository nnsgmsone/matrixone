...
func (e *Exec) compileVTree(vt *vtree.ViewTree, varsMap map[string]int) (*Scope, error) {
	...
	case d > 1 && !isB:
		...
		if s, _, err = e.compileCAQ(vt.FreeVars, vt.Views, varsMap, fvarsMap); err != nil {
			return nil, err
		}
	default:
		return nil, errors.New(errno.SQLStatementNotYetComplete, "not support now")
...
func (e *Exec) compileCAQ(freeVars []string, vs []*vtree.View, varsMap, fvarsMap map[string]int) (*Scope, []string, error) {
	var ss []*Scope
	var fvars []string
	...
	if len(freeVars) == 0 { // only for test
		return nil, nil, errors.New(errno.SQLStatementNotYetComplete, "not support now")
	}
	arg := &times.Argument{
		VarsMap:  varsMap,
		FreeVars: freeVars,
	...
	for i := 0; i < len(vs)-1; i++ {
		...
		if n := len(vs[i].Children); n > 0 {
			s, fvs, err := e.compileCAQ(nil, vs[i].Children, varsMap, fvarsMap)
			if err != nil {
				return nil, nil, err
			}
			ss = append(ss, s)
