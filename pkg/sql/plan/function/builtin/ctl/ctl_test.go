...
func TestHandlerWithServiceTypeNotSupported(t *testing.T) {
	vec1 := vector.New(0, types.New(types.T_varchar, types.MaxVarcharLen, 0, 0))
	require.NoError(t, vec1.Append([]byte("not support service"), false, mpool.MustNewZero()))

	vec2 := vector.New(0, types.New(types.T_varchar, types.MaxVarcharLen, 0, 0))
	require.NoError(t, vec2.Append([]byte("ping"), false, mpool.MustNewZero()))

	vec3 := vector.New(0, types.New(types.T_varchar, types.MaxVarcharLen, 0, 0))
