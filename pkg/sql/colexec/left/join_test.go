...
func init() {
	...
	tcs = []joinTestCase{
		newTestCase([]bool{false}, []types.Type{testType}, []colexec.ResultPos{colexec.NewResultPos(0, 0)},
			[][]*plan.Expr{
				{
					newExpr(0, testType),
				},
...
func hashBuild(t *testing.T, tc joinTestCase) *batch.Batch {
	...
	require.NoError(t, err)
	intputBatch := newBatch(t, tc.flgs, tc.types, tc.proc, Rows)
	tc.proc.Reg.MergeReceivers[0].Ch <- intputBatch
	tc.proc.Reg.MergeReceivers[0].Ch <- nil
	ok, err := hashbuild.Call(0, tc.proc, tc.barg, false, false)
	require.NoError(t, err)
	intputBatch.Clean(tc.proc.Mp())
